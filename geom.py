...
class Interval:
    ...
    def __init__(self, start, end=None, len=None, inclusive=True, sort=False):  # pylint:disable=redefined-builtin # len is really the best name for this
        if end is None and len is None:
            ...
            raise Exception("Cannot specify both end and len:",
                            start, end, len)
...
class CuboidSet:
    ...
    def complement(self):
        """Returns the complement of this set"""
        if not self.cubs:
            # TODO: track the dimension separately so empty complements work
            raise Exception("Cannot determine dimension")
        return CuboidSet._new([infinite_cuboid(len(next(iter(self.cubs)).dims))]) - self
